...
export function setupAartiForm(formElement, onSuccess, onError) {
  ...
  function handleSubmit(event){
    ...
    const errors = [];
    const nameValidaion = validateName(name);
    if(!nameValidaion.valid){
      errors.push(nameValidaion.error);
    }
    ...
    if(!aartiValidation.valid){
      errors.push(aartiValidation.error);
    }
    if(errors.length > 0){
      onError(errors);
    }
...
export function createBookingSummary(booking) {
  ...
  const paarti = document.createElement("p");
  paarti.className = "booking-type";
  paarti.textContent = `Aarti: ${booking.aartiType}`;
  div.appendChild(h3);
  div.appendChild(pname);
  div.appendChild(pdate);
  div.appendChild(paarti);
  return div;
}
