...
export function createPandalElement(pandal) {
  ...
  div.dataset.budget = pandal.budget.toString();
  div.dataset.rating = pandal.rating.toString();
...
export function filterPandalsByZone(container, zone) {
  ...
  return Array.from(container.querySelectorAll(".pandal")).filter((pandal) => pandal.dataset.zone === zone);
}
